...
from .pipeline import YEAR_COL, VISIT_COL
...
def plot_visits_histogram(student_visits: pd.DataFrame, output_dir: Path) -> list[Path]:
    ...
    ax.set_title("Distribucion de visitas por estudiante")
...
def plot_visits_histogram_low_counts(
    ...
    ax.set_title(f"Distribucion de visitas por estudiante (0 a {max_visits_shown})")
    ax.set_xlabel("Visitas")
    ax.set_ylabel("Numero de estudiantes")
...
def plot_visits_continuation_curve(visit_tail: pd.DataFrame, output_dir: Path) -> list[Path]:
    ...
    ax.set_title("Continuacion de visitas")
    ax.set_xlabel("k")
...
def plot_visits_by_year(student_year_visits: pd.DataFrame, output_dir: Path) -> list[Path]:
    ...
    ax.set_title("Visitas del reporte entre estudiantes activos por anio")
    ax.set_xlabel("Anio")
...
def plot_classroom_size_distribution(classroom_summary: pd.DataFrame, output_dir: Path) -> list[Path]:
    ...
    ax.set_title("Distribucion del tamano de aula")
    ax.set_xlabel("Estudiantes por aula")
    ax.set_ylabel("Numero de aulas")
...
def plot_visits_lorenz_curve(lorenz: pd.DataFrame, output_dir: Path) -> list[Path]:
    ...
    ax.set_title("Concentracion de visitas")
    ax.set_xlabel("Proporcion acumulada de estudiantes")
    ax.set_ylabel("Proporcion acumulada de visitas")
    ax.set_xlim(0, 1)
    ax.set_ylim(0, 1)
    ax.legend(frameon=False)
