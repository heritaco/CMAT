...
from .descriptive_pipeline import COLUMN_ROLE_MAP, GRADE_VARIABLE_NOTES
...
def _latex_table(df: pd.DataFrame) -> str:
    ...
    lines.append(" & ".join(str(column) for column in df.columns) + r" \\")
...
def write_tex_snippets(
    ...
    core_counts_path.write_text(_latex_table(core_counts), encoding="utf-8")
    ...
    visits_path.write_text(_latex_table(visits_table), encoding="utf-8")
    ...
    year_path.write_text(_latex_table(year_table), encoding="utf-8")
    ...
    cleaning_path.write_text(_latex_table(cleaning_table), encoding="utf-8")
    ...
    concentration_path.write_text(_latex_table(concentration_table), encoding="utf-8")
    ...
    grade_path.write_text(_latex_table(grade_table), encoding="utf-8")
    ...
    tokens_path.write_text(_latex_table(tokens_table), encoding="utf-8")
