...
Para operacionalizar la comparación "Are they better?", la Figura 3.1.2 define el criterio de agrupamiento por visitas al CMAT (umbral $>3$).
...
El eje horizontal es la calificación (0--10) y el vertical es densidad, mostrando la heterogeneidad de formas entre docentes tras la imputación.
...
\newpage
\section{Cute plot}
En la sección "Cute plot" se reutilizan visualizaciones del análisis por salón y se añade una agregación por número de visitas.
...
\begin{itemize}
  ...
  \item \textbf{Group B (low or no CMAT)}: students with three or fewer
        visits ($n_B = 9377$).
\end{itemize}
...
H_0:\ \mu_A = \mu_B
\]
Students with frequent CMAT use do not differ in their standardized
grades from students with low or no CMAT use.

\textbf{Alternative hypothesis:}
\[
H_1:\ \mu_A \neq \mu_B
\]
There is a difference in mean standardized grades between the two
groups.

\subsection{Parametric Test}

A Welch two-sample $t$-test was conducted comparing standardized
grades across groups. The test yielded:
...
\begin{itemize}
  ...
  \item Mean(A) = 0.34
  \item Mean(B) = -0.08
  \item Difference in means = 0.41
\end{itemize}

\textbf{Conclusion:} Since the $p$-value is effectively zero, the null
hypothesis is rejected. Frequent CMAT users have significantly higher
standardized grades on average.

\subsection{Robust (Non-Parametric) Evidence}

Given the non-normality of standardized grades, robust tests were also
applied. Results were consistent with the parametric finding:

\begin{itemize}
  \item Median(A) = 0.422
  \item Median(B) = 0.185
  \item Difference in medians = 0.237 (CI95\% [0.202, 0.284])
  \item Mann-Whitney U test: $p = 3.224 \times 10^{-50}$
  \item Brunner-Munzel test: $p = 2.023 \times 10^{-57}$
  \item Common-language effect size: CL = 0.641 (CI95\% [0.625, 0.657])
  \item Rank-biserial correlation: $r_{rb} = 0.282$ (CI95\% [0.250, 0.313])
  \item Cliff’s delta: $\delta = 0.283$ (CI95\% [0.234, 0.326])
\end{itemize}

\textbf{Interpretation:}
A student with more than three CMAT visits has a 64\% probability of
having a higher standardized grade than a student with three or fewer
visits. Effect sizes are moderate but consistent.

\subsection{Final Conclusion}

Across both parametric and robust tests, students who attended CMAT
more than three times exhibited significantly higher standardized
grades. The evidence supports the claim that frequent CMAT use is
associated with improved academic performance, although the analysis
